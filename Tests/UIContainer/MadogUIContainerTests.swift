...
class MadogUIContainerTests: MadogKIFTestCase {
    ...
    func testCloseMainReleasesBothContexts() {
        ...
        XCTAssertNil(context)
        XCTAssertNil(modalContext)
        viewTester().waitForLabel(token: "vc1") // Main UI should still be there
    ...
    func testChangeSingleToMulti() {
        ...
        viewTester().waitForTitle(token: "vc2") // Titles should appear in the tab bar
    ...
    func testChangeMultiToSingle() {
        ...
        viewTester().waitForTitle(token: "vc1") // Titles should appear in the tab bar
        viewTester().waitForTitle(token: "vc2")
        XCTAssertNotNil(context)

        context = context?.change(to: .basic, tokenData: .single("vc3"))
        viewTester().waitForAbsenceOfTitle(token: "vc1")
    ...
    func testCloseMultiUIModal() {
        ...
        XCTAssertTrue(context!.closeModal(token: modalToken!, animated: true))
        viewTester().waitForAbsenceOfTitle(token: "vc2")
        viewTester().waitForAbsenceOfTitle(token: "vc3")
    }
